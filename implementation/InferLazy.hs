...
step n (Sub (TVar (Right i)) (TArrow a b) : ws) -- 09
  | not $ mono (TArrow a b) =
    ( n + 2,
      updateBoundInWL (TVar (Right i)) (UB, a1_a2) (addTypsBefore (TVar (Right i)) [a1, a2] ws)
        >>= (\wl' -> Right $ Sub (TArrow a1 a2) (TArrow a b) : wl'),
      "SplitL"
    )
  where
    a1 = TVar (Right n)
...
step n (Sub (TArrow a b) (TVar (Right i)) : ws) -- 10
  | not $ mono (TArrow a b) =
    ( n + 2,
      updateBoundInWL (TVar (Right i)) (LB, a1_a2) (addTypsBefore (TVar (Right i)) [a1, a2] ws)
        >>= (\wl' -> Right $ Sub (TArrow a b) (TArrow a1 a2) : wl'),
      "SplitR"
    )
  where
    a1 = TVar $ Right n
