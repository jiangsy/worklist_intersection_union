...
(T |- ^a <: t)*              = (T with new work from [t/^a]+_{} T)*                                           
(T |- t <: ^a)*              = (T with new work from [t/^a]-_{} T)*                                           
...
What is the termination argument for these 2 rules ...?

(T |- ^a <: t)*              = (T + (([t/^a]+_{} T) - (T |- ^a <: t))*                                           
(T |- t <: ^a)*              = (T + (([t/^a]-_{} T) - (T |- t <: ^a))*

(S | T |- ^a <: t)*          = S, [t / ^a]+ | T ++ (S . (^a <: t)) -- only generates new work if fv(t) in S && t /= S(t)        
...
createSub i a Pos = Sub (TVar (Right i)) a
createSub i a Neg = Sub a (TVar (Right i))

evalSolve :: Int -> Int -> Typ -> Mode -> [Work] -> [Work] -> Writer String Bool
evalSolve n i a m ws ws' = eval n ([ w | w <- ws', not (elem w (createSub i a m : ws))] ++ ws)
...
eval n (w : ws)            = let ps name = tell (printStep (w : ws) name) in 
  case w of
       ...
       (Sub (TVar (Right i)) a) | mono a   -> ps "SolveL" >> evalSolve n i a Pos ws (substWL Pos i a [] ws)
       (Sub a (TVar (Right i))) | mono a   -> ps "SolveR" >> evalSolve n i a Neg ws (substWL Neg i a [] ws)
