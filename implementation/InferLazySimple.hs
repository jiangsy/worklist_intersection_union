...
step n (Sub (TVar (Right i)) (TArrow a b) : ws) -- 09
  | not $ mono (TArrow a b) =
    ( n + 2,
      carryBackInWL (TVar (Right i)) a1_a2 (Sub a1_a2 (TArrow a b) : WExVar (n + 1) [] [] : WExVar n [] [] : ws)
        >>= updateBoundInWL (TVar (Right i)) (UB, a1_a2),
      "SplitL"
    )
  where
    a1 = TVar (Right n)
...
step n (Sub (TArrow a b) (TVar (Right i)) : ws) -- 09
  | not $ mono (TArrow a b) =
    ( n + 2,
      carryBackInWL (TVar (Right i)) a1_a2 (Sub (TArrow a b) a1_a2 : WExVar (n + 1) [] [] : WExVar n [] [] : ws)
        >>= updateBoundInWL (TVar (Right i)) (LB, a1_a2),
      "SplitL"
    )
  where
    a1 = TVar (Right n)
...
step n (Sub (TVar (Right i)) t : ws) -- 11
  | mono t =
    ( n,
      carryBackInWL (TVar (Right i)) t ws
        >>= updateBoundInWL (TVar (Right i)) (UB, t),
      "Solve mono R"
    )
...
step n (Sub t (TVar (Right i)) : ws) -- 12
  | mono t =
    ( n,
      carryBackInWL (TVar (Right i)) t ws
        >>= updateBoundInWL (TVar (Right i)) (LB, t),
      "Solve mono L"
    )
  | otherwise = error $ "Bug: Incorrect " ++ show t ++ " !"
