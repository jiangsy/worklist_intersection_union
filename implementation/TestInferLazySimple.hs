...
import qualified InferLazy as InferS (checkAndShow, chk, chkAndShow)
import InferLazySimple (checkAndShow, chk, chkAndShow, mono)
...
abstractHelper seed TInt typLs =
  if null typLs
    then (TInt, seed)
      ...
      let (x :: Float, nextSeed) = getRandomFloat seed
       in if x < 0.2
            then (TInt, nextSeed)
       ...
       in if
              | x < 0.2 ->
                let (polyt1, nextSeed') = abstractHelper nextSeed t1 typLs
                    (polyt2, nextSeed'') = abstractHelper nextSeed' t2 typLs
                 in (TArrow polyt1 polyt2, nextSeed'')
              | x >= 0.2 && x < 0.95 ->
                ( TForall $ \x ->
                    let (polyt1, nextSeed') = abstractHelper nextSeed t1 (x : typLs)
                        (polyt2, _) = abstractHelper nextSeed' t2 (x : typLs)
                     in TArrow polyt1 polyt2,
                  getNewSeed seed (maxRandomSeedUsed t)
                )
...
monoNTypGen n =
  oneof
    [ liftM2 TArrow (monoNTypGen (n - 1)) (monoNTypGen (n - 1)),
      elements [TBool, TInt]
    ]
