...
(T |- ^a <: t)*              = ([t/^a]_{} T)* \/ ([Bot/^a]_{} T)*                                           
(T |- t <: ^a)*              = ([t/^a]_{} T)* \/ ([Top/^a)_{} T)*                                           
...
Special cases to prevent Top/Bot splits:

[Top/^a]_E (T,^a)           = T,E,^a,{Top <: ^a}                 
[Bot/^a]_E (T,^a)           = T,E,^a,{^a <: Bot}                 
...
-- trying a Fix for Jimmy's problem
--mono TTop         = False
--mono TBot         = False
mono _            = True
