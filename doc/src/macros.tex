% math-mode versions of \rlap, etc
% from Alexander Perlis, "A complement to \smash, \llap, and lap"
%   http://math.arizona.edu/~aprl/publications/mathclap/
\def\clap#1{\hbox to 0pt{\hss#1\hss}}
\def\mathllap{\mathpalette\mathllapinternal}
\def\mathrlap{\mathpalette\mathrlapinternal}
\def\mathclap{\mathpalette\mathclapinternal}
\def\mathllapinternal#1#2{\llap{$\mathsurround=0pt#1{#2}$}}
\def\mathrlapinternal#1#2{\rlap{$\mathsurround=0pt#1{#2}$}}
\def\mathclapinternal#1#2{\clap{$\mathsurround=0pt#1{#2}$}}

% math-mode versions of \rlap, etc
% from Alexander Perlis, "A complement to \smash, \llap, and lap"
%   http://math.arizona.edu/~aprl/publications/mathclap/
\def\clap#1{\hbox to 0pt{\hss#1\hss}}
\def\mathllap{\mathpalette\mathllapinternal}
\def\mathrlap{\mathpalette\mathrlapinternal}
\def\mathclap{\mathpalette\mathclapinternal}
\def\mathllapinternal#1#2{\llap{$\mathsurround=0pt#1{#2}$}}
\def\mathrlapinternal#1#2{\rlap{$\mathsurround=0pt#1{#2}$}}
\def\mathclapinternal#1#2{\clap{$\mathsurround=0pt#1{#2}$}}

\newcommand{\horizontalrule}{
  \begin{center}
    \line(1,0){250}
  \end{center}
}

\newcommand{\code}[1]{\texttt{#1}}
\definecolor{light-gray}{gray}{0.9}
\newcommand{\highlight}[1]{\colorbox{light-gray}{#1}}

\newcommand{\im}[1]{\lvert #1 \rvert}
\newcommand{\powerset}[1]{\mathcal{P}(#1)}
\newcommand{\universe}{\mathbb{U}}

\newcommand{\turns}{\vdash}
\newcommand{\oftype}{\!:\!}
\newcommand{\inference}{\!\Rightarrow\!}
\newcommand{\subtype}{<:}
\newcommand{\commonsuper}{\Uparrow}
\newcommand{\commonsub}{\Downarrow}
\newcommand{\disjoint}{*}
\newcommand{\disjointimpl}{*_\textnormal{i}}
\newcommand{\disjointax}{*_\textnormal{ax}}
\newcommand{\subst}[2]{\lbrack #2 := #1 \rbrack~}

\newcommand{\yields}[1]{\highlight{$\; \hookrightarrow #1$}}

\newcommand{\ftv}[1]{\textsf{ftv}(#1)}

\newcommand{\binderspace}{\,}
\newcommand{\appspace}{\;}

\newcommand{\inter}{\&}
\newcommand{\union}{|}
\newcommand{\for}[2]{\forall #1.\binderspace #2}
\newcommand{\fordis}[3]{\for {(#1 \disjoint #2)} {#3}}
\newcommand{\lam}[2]{\lambda #1.\binderspace #2}
%\newcommand{\lamty}[3]{\lam {(#1 \oftype #2)} #3} no longer using types in lambdas
\newcommand{\lamty}[3]{\lam #1 #3}
\newcommand{\blam}[2]{\Lambda #1.\binderspace #2}
\newcommand{\blamdis}[3]{\blam {(#1\disjoint#2)} #3}
\newcommand{\mergeop}{,,}
\newcommand{\app}[2]{#1 \; #2}
\newcommand{\tapp}[2]{#1 \appspace #2}
\newcommand{\pair}[2]{(#1, #2)}
\newcommand{\proj}[2]{{\code{proj}}_{#1} #2}
\newcommand{\fst}[1]{\app {\code{fst}} {#1}}
\newcommand{\snd}[1]{\app {\code{snd}} {#1}}
\newcommand{\recordType}[2]{\{ #1 : #2 \}}
\newcommand{\recordCon}[2]{\{ #1=#2 \}}
\newcommand{\recordProj}[2]{#1 . #2}

\newcommand{\true}{\code{True}}
\newcommand{\fsub}{$F_{<:}$\xspace}
\newcommand{\fw}{$F_{\omega}$\xspace}
\newcommand{\fsubo}{$F_{<:}^{\omega}$\xspace}
\newcommand{\lami}{$\lambda_{I}$\xspace}
\newcommand{\name}{$\lambda_{I<:}$\xspace}
\newcommand{\unit}{()}
\newcommand{\tyint}{\code{Int}}
\newcommand{\tybool}{\code{Bool}}
\newcommand{\tychar}{\code{Char}}
\newcommand{\tystring}{\code{String}}
\newcommand{\lamint}{\lambda \&}
\newcommand{\toplike}[1]{\rceil #1 \lceil}
\newcommand{\erase}[1]{\lfloor #1 \rfloor}
\newcommand{\andcoerce}[1]{\llbracket #1 \rrbracket}

% existential variables
\newcommand\wh{\widehat}

\newcommand\al{}
\newcommand\bt{}

\RenewDocumentCommand \al {O{}} {\wh\alpha_{#1}}   % \widehat{\alpha} --- subscript is optional \al[1]
\RenewDocumentCommand \bt {O{}} {\wh\beta_{#1}}    % \widehat{\beta}

% Judgements
\newcommand{\jwf}[2]{#1 \turns #2}
\newcommand{\jatomic}[1]{#1 \ \textnormal{ordinary}}
\newcommand{\jtype}[3]{#1 \turns #2 \ \oftype \ #3}
\newcommand{\jinfer}[3]{#1 \turns #2 \ \inference \ #3}
\newcommand{\jcheck}[3]{#1 \turns #2 \ \!\Leftarrow\! \ #3}
\newcommand{\jdis}[3]{#1 \turns #2 \disjoint #3}
\newcommand{\jdisimpl}[3]{#1 \turns #2 \disjointimpl #3}

\newcommand{\reflabel}[1]{\textsc{#1}}

%\newcommand{\name}{$ F_{i} $\xspace}
\newcommand{\namedis}{$ F_{i}^{*} $\xspace}
\newcommand{\oldname}{$ \lambda_{i} $\xspace}

\newcommand{\authornote}[3]{\textcolor{#2}{\textsc{#1}: #3}}
%\newcommand{\authornote}[3]{}
\newcommand\bruno[1]{\authornote{bruno}{red}{#1}}
\newcommand\jimmy[1]{\authornote{jimmy}{blue}{#1}}

% Proofs
\newcommand{\restatableproof}[7]{
  \begin{restatable}[#2]{#1}{#3}
    \label{#4}
    #5
  \end{restatable}%
  \vspace{#7}
  \noindent \emph{Proof.} #6 \hfill\(\Box\)
  %\vspace{-0.51cm}%
  %\begin{proof}
  %  #6
  %\end{proof}
  %\vspace{-0.51cm}
}

\newcommand{\thmprf}[5]{
  \vspace{#1}
  #2
  \vspace{#3}
  \noindent \emph{Proof.} #4 \hfill\(\Box\)
  \vspace{#5}
}

\newcommand{\defaultthmprf}[2]{
  \thmprf{0cm}{#1}{-0.2cm}{#2}{0cm}
}

%\newcommand{\lemmaproof}[5]{
%  \restatableproof{lemma,#1,#2,#3,#4,#5}
%}

%\newcommand{\theoremproof}[5]{
%  \restatableproof{theorem,#1,#2,#3,#4,#5}
%}
