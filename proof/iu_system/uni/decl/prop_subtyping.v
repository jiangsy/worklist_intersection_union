  ...
  - assert ((forall X : atom, X `notin` L1 -> d_sub_tvar_open_inv X (open_typ_wrt_typ_rec 1 S1 T1_1 ^ᵈ X)) \/
            (forall X : atom, X `notin` L1 -> d_sub_tvar_open_inv X (open_typ_wrt_typ_rec 1 S1 T1_2 ^ᵈ X))).
    {
      ...
      - left. intros. inst_cofinites_with X.
        ...
        replace (open_typ_wrt_typ_rec 1 S1 T1_1 ^ᵈ X) with ({`ᵈ X /ᵈ x}(open_typ_wrt_typ_rec 1 S1 T1_1 ^ᵈ x)).
      ...
      - right. intros. inst_cofinites_with X.
        ...
        replace (open_typ_wrt_typ_rec 1 S1 T1_2 ^ᵈ X) with ({`ᵈ X /ᵈ x}(open_typ_wrt_typ_rec 1 S1 T1_2 ^ᵈ x)).
        now apply d_sub_tvar_open_inv_subst_var.
        rewrite subst_tvar_in_typ_open_typ_wrt_typ; auto.
        rewrite (subst_tvar_in_typ_fresh_eq); auto. simpl.
        unfold eq_dec. destruct (EqDec_eq_of_X x x); auto. contradiction.
...
Lemma d_sub_tvar_inv_subst: forall T1 X S1,
    d_sub_tvar_inv T1 -> lc_typ S1 -> d_sub_tvar_inv ({S1 /ᵈ X} T1).
Proof.
...
Proof with eauto with subtyping.
  ...
  - destruct B1.
    + eapply d_sub__alll with (L:=L `union` singleton X) (T1:={T1 /ᵈ X} T0); auto...
    ...
    + eapply d_sub__alll with (L:=L `union` singleton X) (T1:={T1 /ᵈ X} T0); eauto...
      * intros. inst_cofinites_with X0. rewrite typ_subst_open_comm; auto...
        apply ftv_sin_typ_subst_inv; auto...
      * apply d_mono_typ_subst_mono_mono; auto.
        admit. admit.
    ...
    + simpl.
      ...
      -- replace (typ_all ({T1 /ᵈ X} A1)) with ({T1 /ᵈ X} (typ_all A1)) by auto.
      ...
      -- unfold eq_dec. destruct (EqDec_eq_of_X X0 X); auto.
         admit. admit.
      -- replace (if X0 == X then T1 else `ᵈ X0) with ( {T1 /ᵈ X} `ᵈ X0) by auto.
         apply d_wft_typ_subst; auto.
    + apply d_sub_dwft in Hsub as Hwft1. destruct Hwft1 as [_ [HwftS HwftT]]. inversion HwftT.
      eapply d_sub__alll with (L:=L `union` singleton X) (T1:={T1 /ᵈ X} T0); eauto...
      * admit.
...
(* Lemma dneq_all_intersection_union_subst_stv : forall T1 T2 SX,
  ...
  (dneq_all ({T2 /ₛᵈ SX} T1) /\
   ...
   dneq_union ({T2 /ₛᵈ SX} T1)) \/ T1 = typ_svar SX.
...
Proof with subst; eauto using dwf_typ_weaken_head with subtyping.
  ...
  - simpl. inst_cofinites_for d_sub_all; intros SX1 Hfr; inst_cofinites_with SX1.
    + rewrite d_subst_stv_in_typ_open_comm; auto...
      apply fstv_sins_typ_subst_stv_s; auto...
    + rewrite d_subst_stv_in_typ_open_comm; auto...
      apply fstv_sins_typ_subst_stv_s; auto...
    + repeat rewrite d_subst_stv_in_typ_open_comm; eauto...
  ...
  - simpl. destruct (dneq_all_intersection_union_subst_stv T1 T2 SX) as [[? [? ?]] | ?]; eauto...
    + eapply d_sub_alll with (L:=L `union` singleton SX) (T2:={T2 /ₛᵈ SX} T0); eauto...
      * intros. inst_cofinites_with SX0. rewrite d_subst_stv_in_typ_open_comm; auto...
...
(* Inductive d_sub_size : denv -> typ -> typ -> nat -> Prop :=
 ...
 | d_subs__stvar : forall (E:denv) (SX:stypvar) (n:nat),
     ...
     d_sub_size E (typ_svar SX) (typ_svar SX) n
 ...
 | d_subs__all : forall (L:vars) (E:denv) (S1 T1:typ) (n:nat),
     ...
     ( forall SX , SX \notin L -> d_sub_size  ( SX ~ dbind_stvar_empty  ++  E )   (open_typ_wrt_typ  S1   (typ_svar SX) )   (open_typ_wrt_typ  T1  (typ_svar SX) ) n) ->
...
Lemma d_sub_size_rename_stvar : forall E F SX SY A B n,
    F ++ SX ~ ▪ ++ E ⊢ A <: B | n ->
      ...
      (map (d_subst_stv_in_binding (typ_svar SY) SX) F) ++ SY ~ ▪ ++ E ⊢
        {typ_svar SY /ₛᵈ SX} A <: {typ_svar SY /ₛᵈ SX} B | n.
Proof with (simpl in *; eauto using d_wf_env_subst_tvar_typ with sub).
  ...
  - econstructor...
    ...
    forwards* : binds_map_2 (d_subst_stv_in_binding (typ_svar SY) SX) H0.
  - case_if; subst...
    + econstructor.
...
Corollary d_sub_size_rename : forall n X Y D E A B,
    X `notin`  (fstv_in_typ A `union` fstv_in_typ B) ->
    Y `notin` ((dom D) `union` (dom E)) ->
    D ++ X ~ ▪ ++ E ⊢ A ^^ᵈ typ_svar X <: B ^^ᵈ typ_svar X | n ->
    map (d_subst_stv_in_binding (typ_svar Y) X) D ++ Y ~ ▪ ++ E ⊢ A ^^ᵈ typ_svar Y <: B ^^ᵈ typ_svar Y | n.
Proof with eauto.
  intros.
  forwards: d_sub_size_rename_stvar Y H1. solve_notin.
  rewrite 2 d_subst_stv_in_typ_open_typ_wrt_typ in H2...
  simpl in H2. case_if.
  rewrite 2 d_subst_stv_in_typ_fresh_eq in H2...
Qed.
...
Proof with auto with trans.
  ...
  - dependent destruction Hsub1...
    ...
    + dependent destruction Hsub2...
      * econstructor.
        eapply IHn_dsub_size with (S1:=typ_svar SX) (n1:=n2); eauto...
        eapply IHn_dsub_size with (S1:=typ_svar SX) (n1:=n1); eauto...
      * simpl in H. eapply d_sub_union1.
        eapply IHn_dsub_size with (S1:=typ_svar SX) (n1:=n); eauto...
        auto.
      * simpl in H. eapply d_sub_union2.
        eapply IHn_dsub_size with (S1:=typ_svar SX) (n1:=n); eauto...
        auto.
    ...
    + simpl in *. dependent destruction Hsub2...
      ...
      * eapply d_sub_all with (L:=L `union` L0 `union` dom E); auto.
        ...
        eapply IHn_d_typ_order with (S1:= T0 ^^ᵈ typ_svar SX); eauto...
