...
Lemma neq_all_intersection_union_subst_stv : forall A B X,
  lc_typ A -> 
  lc_typ B ->
  neq_all A -> 
  neq_intersection A -> 
  neq_union A ->
  (neq_all ({B ᵗ/ₜ X} A) /\
    neq_intersection ({B ᵗ/ₜ X} A) /\
    neq_union ({B ᵗ/ₜ X} A)) \/ A = ` X.
...
(* Lemma sneq_all_intersection_union_subst_stv : forall A B X,
  lc_typ A -> 
  lc_typ B ->
  sneq_all A ->
  sneq_all ({B ᵗ/ₜ X} A) \/ A = ` X.
Proof with eauto with lngen.
      ...
      * destruct A; simpl in *; try solve [inversion Heq].
        -- destruct n.
          ++ unfold open_typ_wrt_typ in Heq. simpl in Heq.
              subst. dependent destruction H7.
          ++ unfold open_typ_wrt_typ in Heq. simpl in Heq.
             inversion Heq.
...
Lemma wneq_all_subst : forall A B X,
  wneq_all A ->
  (weq_stvar X A -> False) ->
  wneq_all ({B ᵗ/ₜ X} A).
Proof.
  intros. dependent induction H; simpl; auto.
  - destruct_eq_atom. exfalso. apply H0; constructor.
    auto.
  ...
  - constructor. admit.
    admit.
  - constructor.
    apply IHwneq_all1. intros. apply H0. constructor; auto.
    apply IHwneq_all2. intros. apply H0. apply weq_stvar__intersection2. auto.
