...
Theorem trans_ftyp_open_typ_wrt_typ : forall A1 A2,
  trans_ftyp (A1 ᵗ^^ₜ A2) = open_ftyp_wrt_ftyp (trans_ftyp A1) (trans_ftyp A2).
Proof.
...
Theorem trans_ftyp_wf : forall Ψ A,
  Ψ ᵗ⊢ᵈ A -> f_wf_typ (trans_env Ψ) (trans_ftyp A).
Proof.
  intros. induction H; simpl; eauto...
...
Proof with eauto with safety.
  ...
  - apply typing_abs with (L:=dom Ψ).
    ...
    apply trans_ftyp_wf with (Ψ:=Ψ) (T:=typ_all (typ_var_b 0)).
  ...
  - apply typing_abs with (L:=dom Ψ).
    ...
    apply typing_var; eauto.
    apply wf_env_typ; eauto with safety.
    apply trans_ftyp_wf with (T:=typ_var_f X); auto.
  - apply typing_abs with (L:=dom Ψ).
    unfold open_fexp_wrt_fexp. simpl. intros.
    apply typing_var; eauto.
    apply wf_env_typ; eauto with safety.
    apply trans_ftyp_wf with (T:=typ_var_f SX); auto.
  - apply typing_abs with (L:=dom Ψ).
    unfold open_fexp_wrt_fexp. simpl. intros.
    ...
    * apply typing_var; auto.
      admit. (* wf *)
    * apply typing_app with (B1:=trans_ftyp B1).
      -- admit. (* weaken *)
      -- apply typing_var; auto.
       admit. (* wf *)
  - apply typing_abs with (L:=L `union` dom Ψ).
    ...
    replace (ftyp_var_f X) with (trans_ftyp (typ_var_f X)); auto.
    apply typing_app with (B1:=trans_ftyp (A1 ᵗ^^ₜ typ_var_f X)).
    + rewrite <- trans_ftyp_open_typ_wrt_typ with (B1:=B1) (B2:=typ_var_f X).
    admit. (* weaken *)
    + rewrite trans_ftyp_open_typ_wrt_typ.
  ...
  - apply typing_abs with (L:=L `union` dom Ψ).
    ...
    repeat rewrite open_fexp_wrt_fexp_rec_lc_fexp;
    try rewrite open_fexp_wrt_fexp_rec_lc_fexp; auto.
    apply typing_app with (B1:=trans_ftyp (A1 ᵗ^^ₜ B2)).
    + admit. (* weaken *)
    + rewrite trans_ftyp_open_typ_wrt_typ.
    apply typing_tapp; eauto with safety.
    apply typing_var; auto.
  ...
  - apply typing_abs with (L:=dom Ψ).
    ...
    try rewrite open_fexp_wrt_fexp_rec_lc_fexp; auto.
    apply typing_inl.
    + eapply typing_app with (B1:=trans_ftyp A1).
      * admit. (* weaken *)
      * apply typing_var; auto.
        admit. (* wf *)
    + admit. (* wf *)
  ...
  - apply typing_abs with (L:=dom Ψ).
    ...
    repeat rewrite open_fexp_wrt_fexp_rec_lc_fexp;
    try rewrite open_fexp_wrt_fexp_rec_lc_fexp; auto.
    apply typing_case with (B1:=trans_ftyp A1) (B2:=trans_ftyp A2)
...
Proof.
  ...
  - apply lc_ftyp_prod.
    + admit.
    + apply trans_ftyp_lc_ftyp. apply d_wf_typ_lc_typ with (Ψ:=Ψ); auto.
  - unfold open_fexp_wrt_fexp. simpl. intros.
    apply lc_fexp_app; eauto with safety.
    rewrite open_fexp_wrt_fexp_rec_lc_fexp; auto.
  - apply lc_ftyp_prod.
...
Theorem infabs_sound_f : forall Ψ A B C Co,
  ...
  typing (trans_env Ψ) Co (ftyp_arrow (trans_ftyp A) (ftyp_arrow (trans_ftyp B) (trans_ftyp C))).
Proof with eauto with safety.
  intros. induction H; simpl; eauto.
  - apply typing_abs with (L:=dom Ψ).
    unfold open_fexp_wrt_fexp. simpl. intros.
  ...
  - apply typing_abs with (L:=dom Ψ).
    ...
    rewrite open_fexp_wrt_fexp_rec_lc_fexp; auto.
    apply typing_app with (B1:=trans_ftyp (open_typ_wrt_typ B1 B2)).
    + admit. (* weaken *)
    + rewrite trans_ftyp_open_typ_wrt_typ. apply typing_tapp.
      * apply trans_ftyp_lc_ftyp; auto.
      * apply typing_var; auto. admit. (* wf *)
  - apply typing_abs with (L:=dom Ψ).
    unfold open_fexp_wrt_fexp. simpl. intros.
    assert (Hlc: lc_fexp Co).
    ...
    + admit. (* weaken *)
    + apply typing_proj1 with (B2:=trans_ftyp B2).
      apply typing_var; auto. admit. (* wf *)
  - apply typing_abs with (L:=dom Ψ).
    unfold open_fexp_wrt_fexp. simpl. intros.
    assert (Hlc: lc_fexp Co).
    ...
    + admit. (* weaken *)
    + apply typing_proj2 with (B1:=trans_ftyp B1).
      apply typing_var; auto. admit. (* wf *)
  - apply typing_abs with (L:=dom Ψ).
    unfold open_fexp_wrt_fexp. simpl. intros.
    ...
    apply typing_case with
      ...
      (B1:=trans_ftyp B1) (B2:=trans_ftyp B2).
    + apply typing_var; auto. admit. (* wf *)
    + unfold open_fexp_wrt_fexp. simpl. intros.
      rewrite open_fexp_wrt_fexp_rec_lc_fexp; auto.
      apply typing_inl.
      * apply typing_app with (B1:=trans_ftyp T3).
        -- admit. (* weaken *)
        -- apply typing_proj1 with (B2:=trans_ftyp T5).
           apply typing_var; auto. admit. (* wf *)
      * admit. (* wf *)
    + unfold open_fexp_wrt_fexp. simpl. intros.
      rewrite open_fexp_wrt_fexp_rec_lc_fexp; auto.
      apply typing_inr.
      * apply typing_app with (B1:=trans_ftyp T5).
        -- admit. (* weaken *)
        -- apply typing_proj2 with (B1:=trans_ftyp T3).
           apply typing_var; auto. admit. (* wf *)
      * admit. (* wf *)
Admitted.
...
Proof.
  ...
  - split; apply lc_fexp_abs; eauto with safety;
    unfold open_fexp_wrt_fexp; simpl; auto.
  - split; apply lc_fexp_abs; eauto with safety.
    + replace (ftyp_all (trans_ftyp B1)) with (trans_ftyp (typ_all B1)); eauto.
      apply trans_ftyp_lc_ftyp. eauto.
    + unfold open_fexp_wrt_fexp. simpl. auto.
    + replace (ftyp_all (trans_ftyp B1)) with (trans_ftyp (typ_all B1)); eauto.
      apply trans_ftyp_lc_ftyp. eauto.
    + unfold open_fexp_wrt_fexp. simpl. intros.
      apply lc_fexp_tapp; eauto. apply trans_ftyp_lc_ftyp. eauto. 
Admitted.
