...
Theorem trans_env_notin_dom : forall Ψ X,
  X `notin` dom Ψ -> X `notin` dom (trans_env Ψ).
Proof.
...
Inductive d_inftapp_elab : denv -> typ -> typ -> typ -> fexp -> fexp -> Prop := 
  ...
  | inftapp_elab_intersection1 : forall (Ψ:denv) (A1 A2 B C1:typ) (co1 co2:fexp),
      d_wf_typ Ψ A2 ->
      d_inftapp_elab Ψ A1 B C1 co1 co2 ->
      d_inftapp_elab Ψ (typ_intersection A1 A2) B C1
        (fexp_abs (trans_typ (typ_intersection A1 A2))
                  (fexp_app co1 (fexp_proj1 (fexp_var_b 0))))
        co2
  | inftapp_elab_intersection2 : forall (Ψ:denv) (A1 A2 B C2:typ) (co1 co2:fexp),
      d_wf_typ Ψ A1 ->
      d_inftapp_elab Ψ A2 B C2 co1 co2 ->
      d_inftapp_elab Ψ (typ_intersection A1 A2) B C2
        (fexp_abs (trans_typ (typ_intersection A1 A2))
                  (fexp_app co1 (fexp_proj2 (fexp_var_b 0))))
        co2
  | inftapp_elab_intersection3 : forall (Ψ:denv) (A1 A2 B C1 C2:typ) (co1 co2 co3 co4:fexp),
      d_inftapp_elab Ψ A1 B C1 co1 co2 ->
  ...
  | inftapp_elab_union : forall (Ψ:denv) (A1 A2 B C1 C2:typ) (co1 co2 co3 co4:fexp),
      d_inftapp_elab Ψ A1 B C1 co1 co2 ->
      d_inftapp_elab Ψ A2 B C2 co3 co4 ->
...
Notation "Ψ ⊢ A ○ B ⇒⇒ C ↪ co1 | co2" :=
    (d_inftapp_elab Ψ A B C co1 co2) 
      (at level 65, A at next level, B at next level, C at next level, no associativity) : type_scope. 
...
Inductive d_chk_inf_elab : denv -> exp -> typing_mode -> typ -> fexp -> Prop :=
  ...
  | d_chk_inf_elab__inf_tabs : forall (L:vars) (Ψ:denv) (e:exp) (A:typ) (eᶠ:fexp),
      ...
      d_chk_inf_elab Ψ (exp_tabs (exp_anno e A)) typingmode__inf (typ_all A)
        (fexp_tabs eᶠ)
  | d_chk_inf_elab__inf_tapp : forall (Ψ:denv) (e:exp) (A B C:typ) (eᶠ co1 co2:fexp),
      d_wf_typ Ψ B ->
      d_chk_inf_elab Ψ e typingmode__inf A eᶠ ->
      d_inftapp_elab Ψ A B C co1 co2 ->
      d_chk_inf_elab Ψ (exp_tapp e B) typingmode__inf C
        (fexp_app co2 (fexp_app co1 eᶠ))
  ...
  | d_chk_inf_elab__chk_abs : forall (L:vars) (Ψ:denv) (e:exp) (A B:typ) (eᶠ:fexp),
      ...
      d_chk_inf_elab Ψ (exp_abs e) typingmode__chk (typ_arrow A B)
        (fexp_abs (trans_typ A) eᶠ)
  | d_chk_inf_elab__chk_sub : forall (Ψ:denv) (e:exp) ( A B:typ) (eᶠ co:fexp),
      d_chk_inf_elab Ψ e typingmode__inf A eᶠ ->
      d_sub_elab Ψ A B co ->
...
Lemma d_sub_elab_rename_dtvar : forall Ψ1 Ψ2 A B b X Y co,
  b = □ \/ b = ■ ->
  Ψ2 ++ (X , b) :: Ψ1  ⊢ A <: B ↪ co ->
  Y ∉ (dom (Ψ2 ++ (X, b) :: Ψ1)) ->
  (map (subst_tvar_in_dbind `Y X) Ψ2) ++ (Y , b) :: Ψ1  ⊢ {`Y ᵗ/ₜ X} A <: {`Y ᵗ/ₜ X} B ↪ subst_typ_in_fexp (ftyp_var_f Y) X co.
Proof with eauto 7 using d_wf_typ_rename_dtvar, d_wf_env_rename_dtvar.
  intros. dependent induction H0; eauto; try solve [simpl; repeat rewrite trans_typ_rename_tvar; eauto using d_wf_typ_rename_dtvar, d_wf_env_rename_dtvar].
  - admit.
  - admit.
  - admit.
  - admit.
...
Lemma d_sub_elab_rename_tvar_cons : forall Ψ A B X Y co b,
  b = □ \/ b = ■ ->
  Y `notin` dom ((X, b) :: Ψ) ->
  (X , b) :: Ψ ⊢ A <: B ↪ co ->
  (Y , b) :: Ψ ⊢ {`Y ᵗ/ₜ X} A <: {`Y ᵗ/ₜ X} B ↪ subst_typ_in_fexp (ftyp_var_f Y) X co.
Proof.
...
Theorem d_sub_elab_sound_f : forall Ψ A B co,
  Ψ ⊢ A <: B ↪ co -> ᵉ⟦ Ψ ⟧ ⊢ co : ftyp_arrow ᵗ⟦ A ⟧ ᵗ⟦ B ⟧.
Proof with eauto 4.
  intros. induction H...
Admitted.


Theorem d_infabs_elab_sound_f : forall Ψ A B C co,
  Ψ ⊢ A ▹ B → C ↪ co -> 
  ᵉ⟦ Ψ ⟧ ⊢ co : ftyp_arrow ᵗ⟦ A ⟧ (ftyp_arrow ᵗ⟦ B ⟧ ᵗ⟦ C ⟧).
Proof.
Admitted.

Theorem d_inftapp_elab_sound_f : forall Ψ A B C co1 co2,
  Ψ ⊢ A ○ B ⇒⇒ C ↪ co1 | co2 ->
  exists D : ftyp, ᵉ⟦ Ψ ⟧ ⊢ co1 : ftyp_arrow ᵗ⟦ A ⟧ D /\ ᵉ⟦ Ψ ⟧ ⊢ co2 : ftyp_arrow D ᵗ⟦ C ⟧.
Proof.
Admitted.
...
Theorem d_chk_inf_elab_sound_f : forall Ψ e A eᶠ mode,
  d_chk_inf_elab Ψ e mode A eᶠ -> ᵉ⟦ Ψ ⟧ ⊢ eᶠ : ᵗ⟦ A ⟧.
Proof.
Admitted.
...
Theorem inftapp_elab_lc_fexp : forall Ψ A B C co1 co2,
  d_inftapp_elab Ψ A B C co1 co2 -> lc_fexp co1 /\ lc_fexp co2.
Proof.
...
Theorem inftapp_sound_f : forall Ψ A B C co1 co2,
  d_inftapp_elab Ψ A B C co1 co2 -> exists D,
  typing (trans_env Ψ) co1 (ftyp_arrow (trans_typ A) D) /\
    typing (trans_env Ψ) co2 (ftyp_arrow D (trans_typ C)).
Proof with eauto with safety.
  intros. induction H; simpl; eauto.
