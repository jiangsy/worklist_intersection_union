(* generated by Ott 0.32, locally-nameless lngen from: uni_monoiu/language.ott *)
Require Import Bool.
...
Fixpoint close_conts_wrt_typ_rec (k:nat) (A_5:var) (cs5:conts) {struct cs5}: conts :=
  match cs5 with
...
with close_contd_wrt_typ_rec (k:nat) (A5:var) (cd5:contd) {struct cd5}: contd :=
  match cd5 with
...
Fixpoint subst_typ_in_contd (A5:typ) (X5:typvar) (cd5:contd) {struct cd5} : contd :=
  ...
  | (contd_infapp e cs) => contd_infapp (subst_typ_in_exp A5 X5 e) (subst_typ_in_conts A5 X5 cs)
  | (contd_infproj A cs) => contd_infproj (subst_typ_in_typ A5 X5 A) (subst_typ_in_conts A5 X5 cs)
...
Fixpoint subst_exp_in_contd (e5:exp) (x5:expvar) (cd5:contd) {struct cd5} : contd :=
  ...
  | (contd_infapp e cs) => contd_infapp (subst_exp_in_exp e5 x5 e) (subst_exp_in_conts e5 x5 cs)
  | (contd_infproj A cs) => contd_infproj A (subst_exp_in_conts e5 x5 cs)
...
(* defns J_d_wneq_all *)
Inductive d_wneq_all : denv -> typ -> Prop :=    (* defn d_wneq_all *)
 | d_wneq_all__unit : forall (Ψ:denv),
     d_wneq_all Ψ typ_unit
 | d_wneq_all__top : forall (Ψ:denv),
     d_wneq_all Ψ typ_top
 | d_wneq_all__label : forall (Ψ:denv) (l:label),
     d_wneq_all Ψ (typ_label l)
 | d_wneq_all__tvar : forall (Ψ:denv) (X:typvar),
      binds ( X )  ( dbind_tvar_empty ) ( Ψ )  ->
     d_wneq_all Ψ (typ_var_f X)
 | d_wneq_all__arrow : forall (Ψ:denv) (A1 A2:typ),
     lc_typ A1 ->
     lc_typ A2 ->
     d_wneq_all Ψ (typ_arrow A1 A2)
 | d_wneq_all__union1 : forall (Ψ:denv) (A1 A2:typ),
     lc_typ A2 ->
     d_wneq_all Ψ A1 ->
     d_wneq_all Ψ (typ_union A1 A2)
 | d_wneq_all__union2 : forall (Ψ:denv) (A1 A2:typ),
     lc_typ A1 ->
     d_wneq_all Ψ A2 ->
     d_wneq_all Ψ (typ_union A1 A2)
 | d_wneq_all__intersection2 : forall (Ψ:denv) (A1 A2:typ),
     d_wneq_all Ψ A1 ->
     d_wneq_all Ψ A2 ->
     d_wneq_all Ψ (typ_intersection A1 A2).
...
(* defns J_a_wneq_all *)
Inductive a_wneq_all : aenv -> typ -> Prop :=    (* defn a_wneq_all *)
 | a_wneq_all__unit : forall (Σ:aenv),
     a_wneq_all Σ typ_unit
 | a_wneq_all__top : forall (Σ:aenv),
     a_wneq_all Σ typ_top
 | a_wneq_all__label : forall (Σ:aenv) (l:label),
     a_wneq_all Σ (typ_label l)
 ...
 | a_wneq_all__etvar : forall (Σ:aenv) (X:typvar),
     ...
     a_wneq_all Σ (typ_var_f X)
 | a_wneq_all__arrow : forall (Σ:aenv) (A1 A2:typ),
     lc_typ A1 ->
     lc_typ A2 ->
     a_wneq_all Σ (typ_arrow A1 A2)
 | a_wneq_all__union1 : forall (Σ:aenv) (A1 A2:typ),
     lc_typ A2 ->
     a_wneq_all Σ A1 ->
     a_wneq_all Σ (typ_union A1 A2)
 | a_wneq_all__union2 : forall (Σ:aenv) (A1 A2:typ),
     lc_typ A1 ->
     a_wneq_all Σ A2 ->
     a_wneq_all Σ (typ_union A1 A2)
 | a_wneq_all__intersection2 : forall (Σ:aenv) (A1 A2:typ),
     a_wneq_all Σ A1 ->
     a_wneq_all Σ A2 ->
     a_wneq_all Σ (typ_intersection A1 A2).
...
Inductive a_mono_typ : aenv -> typ -> Prop :=    (* defn a_mono_typ *)
 ...
 | a_mono_typ__intersection : forall (Σ:aenv) (A1 A2:typ),
     a_mono_typ Σ A1 ->
     a_mono_typ Σ A2 ->
     a_mono_typ Σ (typ_intersection A1 A2)
 | a_mono_typ__union : forall (Σ:aenv) (A1 A2:typ),
     a_mono_typ Σ A1 ->
     a_mono_typ Σ A2 ->
     a_mono_typ Σ (typ_union A1 A2).
...
(** infrastructure *)
#[export] Hint Constructors s_in neq_abs d_wneq_all d_wf_typ d_wf_exp d_wf_tenv d_wf_env d_mono_typ d_wf_conts d_wf_contd d_wf_work d_wf_twl d_wf_wl a_wneq_all a_wf_typ a_mono_typ a_wf_exp a_wf_conts a_wf_contd a_wf_work a_wf_wwl a_wf_twl a_wf_wl a_wf_env lc_typ lc_exp lc_contd lc_conts lc_dbind lc_abind lc_work lc_aworklist lc_dworklist : core.
