...
Proof with eauto using wf_ss_strengthen_etvar.
  ...
  - simpl in Hinsta. 
    ...
    inst_cofinites_with X0; rewrite subst_typ_in_typ_open_typ_wrt_typ_fresh2...
...
Proof with eauto.
    ...
    + apply IHΓ2 in Htranswl as Hws; auto... 
      ...
      * econstructor; auto.
        ...
        -- intros. apply Hbinds...
           ...
           apply subst_typ_in_typ_fresh_same in H0...
        -- eapply trans_typ_etvar_subst_same_ss; eauto. 
  ...
  - dependent destruction Htranswl.
    ...
    + simpl. constructor; auto.
      ...
      * intros. apply Hbinds...
        unfold not. intros. subst.
        apply subst_typ_in_work_fresh_same in H0...
...
Proof with eauto.
  ...
  - inst_cofinites_for trans_typ__all; intros;
    inst_cofinites_with X0; repeat rewrite subst_typ_in_typ_open_typ_wrt_typ_fresh2... 
    + apply s_in_subst_inv...
    + rewrite_env (((X0, □) :: θ2) ++ θ1).
      eapply H1; eauto...
...
Lemma wf_ss_tvar_etvar : forall θ1 θ2 X T,
  ...
  ⌈ θ1 ⌉ᵈ ᵗ⊢ᵈₘ T ->
  wf_ss (map (subst_typ_in_dbind T X) θ2 ++ (X , dbind_typ T) :: θ1).
Proof with eauto.
  intros. induction θ2; simpl; auto.
  - dependent destruction H. constructor; auto.
  - destruct a as [Y]; destruct d; auto; dependent destruction H; eauto.
    + econstructor; eauto.
      replace (ss_to_denv (map (subst_typ_in_dbind T X) θ2 ++ (X, dbind_typ T) :: θ1)) with 
        ((map (subst_typ_in_dbind T X) (ss_to_denv θ2) ++ ss_to_denv ((X, dbind_typ T) :: θ1))).
      * simpl.
...
Lemma trans_typ_tvar_etvar : forall θ1 θ2 Aᵃ Aᵈ Tᵃ Tᵈ X,
  ...
  ⌈ θ1 ⌉ᵈ ᵗ⊢ᵈₘ Tᵈ ->
  θ1 ᵗ⊩ Tᵃ ⇝ Tᵈ ->
  map (subst_typ_in_dbind Tᵈ X) θ2 ++ (X , dbind_typ Tᵈ) :: θ1 ᵗ⊩ Aᵃ ⇝ {Tᵈ ᵗ/ₜ X} Aᵈ.
Proof with eauto using wf_ss_tvar_etvar, d_mono_typ_strengthen_to_wf_env.
  ...
  - econstructor; eauto...
    + apply binds_remove_mid in H0...
      ...
      destruct H0.
      * apply binds_app_iff. left. apply binds_map with (f:=(subst_typ_in_dbind Tᵈ X)) in H0...
  ...
  - inst_cofinites_for trans_typ__all; intros; 
    ...
    rewrite_env (map (subst_typ_in_dbind Tᵈ X) ((X0, □) :: θ2) ++ (X, dbind_typ Tᵈ) :: θ1).
    eapply H1; eauto.
Qed.
...
Proof.
  intros. rewrite_env ((map (subst_typ_in_dbind Tᵈ X) nil) ++ (X ,  dbind_typ Tᵈ) :: θ). 
...
Proof.
  ...
  apply subst_typ_in_typ_fresh_same in H0; auto.
Qed.
 ...
 - solve_awl_trailing_etvar.
  ...
  - solve_awl_trailing_etvar. 
    ...
    + apply wf_ss_binds_mono_typ in H1 as Hmonoa...
      ...
      destruct (X0 == X).
      * subst. assert ((work_sub B1 A1 ⫤ᵃ work_sub A2 B2 ⫤ᵃ Γ0) ⟶ᵃʷ⁎⋅)...
        apply IHd_wl_red...
        ...
        -- constructor...
           eapply a_wl_red_aworklist_trailing_sub_weaken with (Γ:=work_sub B1 A1 ⫤ᵃ work_sub A2 B2 ⫤ᵃ Γ0); eauto.
      ...
      * destruct_d_wf_wl. destruct_a_wf_wl. 
        ...
        -- eapply aworklist_subst_transfer_same_dworklist_rev_exist with (T:=`X0) (Tᵈ:= typ_arrow A1 B2) 
           ...
           destruct Htransws as [Γ1 [Γ2 [θ' [Hsubst [Htransws [Hbinds Hwfss]]]]]].
           apply a_wl_red__sub_etvarmono1 with (Γ1:=Γ1) (Γ2:=Γ2); eauto.
           eapply a_wl_red_aworklist_trailing_sub_weaken with (Γ:=work_sub A1 A1 ⫤ᵃ work_sub B2 B2 ⫤ᵃ (subst_typ_in_aworklist ` X0 X Γ2 ⧺ Γ1)); eauto.
    ...
    + apply wf_ss_binds_mono_typ in H2 as Hmonob...
      ...
      * destruct_d_wf_wl.
        ...
        ++ destruct Htransws as [Γ1 [Γ2 [θ' [Hsubst [Htransws [Hbinds Hwfss]]]]]].
           ...
           eapply a_wl_red_aworklist_trailing_sub_weaken with 
            (Γ:=work_sub A1 A1 ⫤ᵃ work_sub B2 B2 ⫤ᵃ (subst_typ_in_aworklist (typ_arrow A1ᵃ B2ᵃ) X Γ2 ⧺ Γ1)); eauto.
      ...
      * inst_cofinites_for a_wl_red__sub_arrow2... 
        ...
        -- simpl. destruct_eq_atom.
           ...
           ++ assert (binds X1 abind_etvar_empty (awl_to_aenv (subst_typ_in_aworklist (typ_arrow ` X1 ` X2) X Γ3 ⧺ Γ2)))
                by (eapply aworklist_subst_binds_same_atvar with (Γ:=(X2 ~ᵃ ⬒ ;ᵃ X1 ~ᵃ ⬒ ;ᵃ Γ0)); eauto).
              assert (binds X2 abind_etvar_empty (awl_to_aenv (subst_typ_in_aworklist (typ_arrow ` X1 ` X2) X Γ3 ⧺ Γ2)))
                by (eapply aworklist_subst_binds_same_atvar with (Γ:=(X2 ~ᵃ ⬒ ;ᵃ X1 ~ᵃ ⬒ ;ᵃ Γ0)); eauto).
  ...
  - solve_awl_trailing_etvar. 
    destruct_trans.
    + solve_binds_nonmono_contradiction.
    + destruct_a_wf_wl. dependent destruction H5. 
      pick fresh X and apply a_wl_red__sub_alll.
      inst_cofinites_with X.
      * eapply trans_wl_d_wneq_all_a_wneq_all...
      * inst_cofinites_with X.
        apply IHd_wl_red.
        ...
        -- exists ((X, dbind_typ T) :: θ0).
           ...
           ++ eapply trans_typ_tvar_etvar_cons with (θ:=θ0) (Tᵃ:=T) (Tᵈ:=T) in H3; eauto...
              rewrite subst_typ_in_typ_open_typ_wrt_typ_tvar2 in H3...
