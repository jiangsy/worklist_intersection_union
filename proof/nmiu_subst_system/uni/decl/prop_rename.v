...
Proof with eauto using d_wf_typ_rename_var, d_wf_env_rename_var, d_mono_typ_rename_var.
  intros. dependent induction H; eauto...
  - inst_cofinites_for d_sub__all; intros; inst_cofinites_with X. inversion H.
    + left. inversion H4. split.
...
Lemma typ_bot_like_rename : forall A X Y,
  typ_bot_like A ->
  typ_bot_like ({`Y ᵗ/ₜ X} A).
Proof.
  intros. induction H; simpl; eauto.  
  - auto using lc_typ_subst.
  - auto using lc_typ_subst.
  - inst_cofinites_for typ_bl__all. intros. inst_cofinites_with X0.
...
Proof with eauto using d_wf_typ_rename_dtvar, d_mono_typ_rename_tvar, d_wf_env_rename_dtvar.
  ...
  - inst_cofinites_for d_sub__all; intros; inst_cofinites_with X0; repeat rewrite subst_tvar_in_typ_open_typ_wrt_typ_fresh2...
    destruct H2.
    + left. destruct H2. split; rewrite_env (map (subst_tvar_in_dbind ` Y X) ((X0 ~ □) ++ Ψ2) ++ (Y, b) :: Ψ1 );
      apply d_sneq_stvar_rename_dtvar; simpl...
    + right. replace (typ_all ({` Y ᵗ/ₜ X} A)) with ({` Y ᵗ/ₜ X} (typ_all A)) by auto...
      apply typ_bot_like_rename; eauto.
    + rewrite_env (map (subst_tvar_in_dbind ` Y X) ((X0 ~ ■) ++ Ψ2) ++ (Y, b) :: Ψ1 )...
      eapply H1... simpl... apply d_wf_env__stvar... 
