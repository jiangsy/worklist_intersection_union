...
Proof with auto with weaken.
  ...
  - apply d_sub__all with (L :=  L `union` dom (Ψ3 ++ Ψ2 ++ Ψ1)); intros X Fr; inst_cofinites_with X; auto...
    destruct H. left.  rewrite_env ((X ~ □ ++ Ψ3) ++ Ψ2 ++ Ψ1). split; apply d_sneq_stvar_weaken; simpl; intuition...
    right; auto.
    eapply H1 with (Ψ1 := Ψ1) (Ψ3 := (X, ■) :: Ψ3); simpl; auto.
    apply d_wf_env__stvar; auto.
  - apply d_sub__alll with (T := T) (L :=  L `union` dom (Ψ3 ++ Ψ2 ++ Ψ1)); auto...
    intros. inst_cofinites_with X. rewrite_env ((X ~ □ ++ Ψ3) ++ Ψ2 ++ Ψ1).
    + destruct H2.  
      * left. apply d_sneq_stvar_weaken; simpl...
      * right. auto.
...
Lemma neq_all_intersection_union_subst_stv : forall T1 T2 X,
  lc_typ T1 -> lc_typ T2 ->
...
Lemma d_mono_typ_bot_like_false : forall Ψ T,
  d_mono_typ Ψ T ->
  typ_bot_like T ->
  False.
Proof.
...
#[local] Hint Constructors typ_bot_like : core.

Lemma typ_bot_like_subst_inv : forall X Ψ A T,
  lc_typ A ->
  Ψ ᵗ⊢ᵈₘ T ->
  typ_bot_like ({T ᵗ/ₜ X} A) ->
  typ_bot_like A.
Proof with eauto.
  intros. dependent induction H; eauto.
  - simpl in H1. destruct_eq_atom.
    + exfalso. eapply d_mono_typ_bot_like_false; eauto. 
    + dependent destruction H1.
  - simpl in H2. inversion H2.
  - dependent destruction H2.
    inst_cofinites_for typ_bl__all; intros.
...
Lemma typ_bot_like_subst : forall X Ψ A B,
  lc_typ A ->
  Ψ ᵗ⊢ᵈ B ->
  typ_bot_like A ->
  typ_bot_like ({B ᵗ/ₜ X} A).
Proof with eauto.
  intros. dependent induction H; eauto.
  - dependent destruction H1. 
  - dependent destruction H2. 
  - dependent destruction H2. simpl. inst_cofinites_for typ_bl__all; intros.
    rewrite subst_tvar_in_typ_open_typ_wrt_typ_fresh2; eauto.
  - dependent destruction H2. simpl. eauto.
  - dependent destruction H2; simpl; eauto using lc_typ_subst.
Qed.

Lemma typ_bot_like_lc_typ : forall A,
  typ_bot_like A -> lc_typ A.
Proof.
  intros. induction  H; auto.
Qed.

#[local] Hint Resolve typ_bot_like_lc_typ : core.

Lemma typ_bot_like_all_inv : forall Ψ A T,
  Ψ ᵗ⊢ᵈₘ T ->
  typ_bot_like (A ᵗ^^ₜ T) ->
  typ_bot_like (typ_all A).
Proof.
  intros. inst_cofinites_for typ_bl__all. intros.
  eapply typ_bot_like_subst_inv with (X:=X); eauto.
  - apply typ_bot_like_lc_typ in H0. eapply lc_typ_subst_inv with (T:=T) (X:=X); eauto.
    rewrite subst_tvar_in_typ_open_typ_wrt_typ_tvar2; eauto.
  - rewrite subst_tvar_in_typ_open_typ_wrt_typ_tvar2; eauto.
Qed.

Lemma d_sub_bot_like_bot_like : forall Ψ A B,
  Ψ ⊢ A <: B ->
  typ_bot_like B ->
  typ_bot_like A.
Proof.
  intros. induction H; eauto; try solve [dependent destruction H0; eauto].
  - dependent destruction H0. inst_cofinites_for typ_bl__all. intros. inst_cofinites_with X.
    auto.
  - eapply typ_bot_like_all_inv; eauto.
Qed.

Lemma d_sneq_stvar_subst_stvar : forall Ψ1 Ψ2 A B X,
  uniq (Ψ2 ++ X ~ ■ ++ Ψ1) ->
  d_sneq_stvar (Ψ2 ++ X ~ ■ ++ Ψ1) A ->
  Ψ1 ᵗ⊢ᵈ B ->
  d_sneq_stvar (map (subst_tvar_in_dbind B X) Ψ2 ++ Ψ1) ({B ᵗ/ₜ X} A).
Proof.
  ...
  - simpl. destruct_eq_atom.
    ...
    + apply binds_remove_mid_diff_bind in H0.
      ...
      destruct H0. apply binds_app_2. apply binds_map_2 with (f:=(subst_tvar_in_dbind B X)) in H0. simpl in H0. auto.
      apply binds_app_3. auto.
      solve_false.
  - simpl. constructor; eauto using lc_typ_subst.
  - simpl. constructor; eauto using lc_typ_subst.
    replace (typ_all ({B ᵗ/ₜ X} A)) with ({B ᵗ/ₜ X} (typ_all A)) by auto.
    apply lc_typ_subst; eauto.
  - simpl. constructor; eauto using lc_typ_subst.
  - simpl. constructor; eauto using lc_typ_subst.
Qed.
    
Lemma d_mono_typ_sneq_stvar : forall Ψ T,
  d_mono_typ Ψ T ->
  d_sneq_stvar Ψ T.
Proof.
...
Lemma d_sneq_stvar_subst_tvar : forall Ψ1 Ψ2 A T X,
  uniq (Ψ2 ++ X ~ □ ++ Ψ1) ->
  d_sneq_stvar (Ψ2 ++ X ~ □ ++ Ψ1) A ->
  Ψ1 ᵗ⊢ᵈₘ T ->
  d_sneq_stvar (map (subst_tvar_in_dbind T X) Ψ2 ++ Ψ1) ({T ᵗ/ₜ X} A).
Proof.
  intros. dependent induction H0; eauto.
  - simpl. destruct_eq_atom.
    + eapply d_mono_typ_sneq_stvar. apply d_mono_typ_weaken_app. auto. 
    + apply binds_remove_mid in H0.
      constructor.
      apply binds_app_iff in H0.
      destruct H0. apply binds_app_2. apply binds_map_2 with (f:=(subst_tvar_in_dbind T X)) in H0. simpl in H0. auto.
      apply binds_app_3. auto.
      solve_false.
  - simpl. constructor; eauto using lc_typ_subst.
  - simpl. constructor; eauto using lc_typ_subst.
    ...
    + intros. dependent destruction H1; rename x into Heq.
      ...
      * destruct A; simpl in *; try solve [ inversion Heq].
        -- destruct n.
            ...
            ++ unfold open_typ_wrt_typ in Heq. simpl in *.
                inversion Heq.
        -- inversion Heq. subst. inst_cofinites_for typ_bl__all; intros.
           fold open_typ_wrt_typ_rec.
           eapply typ_bot_like_subst_inv with (X:=X) (T:=T0); eauto.
      ...
      * destruct A; simpl in *; try solve [inversion Heq].
        -- destruct n.
           ...
           ++ inversion Heq.
        -- inversion Heq. subst. apply typ_bl__intersection1.
           ++ apply d_sub_d_wf_typ1 in H1; eauto. 
           ++ eapply IHn2; eauto. unfold open_typ_wrt_typ. lia.  unfold open_typ_wrt_typ. lia.
      * destruct A; simpl in *; try solve [inversion Heq].
        -- destruct n.
           ++ unfold open_typ_wrt_typ in Heq. simpl in Heq.
              subst. dependent destruction H3.
           ++ inversion Heq.
        -- inversion Heq. subst. apply typ_bl__intersection2. apply d_sub_d_wf_typ1 in H1; eauto. 
            eapply IHn2; eauto.
...
Theorem d_sub_open_mono_stvar_sub_bot: forall n1 n2 Ψ A T X,
  d_typ_order (A ᵗ^^ₜ T) < n1 ->
  d_typ_size (A ᵗ^^ₜ T) < n2 ->
  X ~ ■ ∈ᵈ Ψ ->
  Ψ ⊢ A ᵗ^^ₜ T <: typ_var_f X ->
  Ψ ᵗ⊢ᵈₘ T ->
  d_sneq_stvar Ψ ( A ᵗ^^ₜ T) ->
  typ_bot_like (A ᵗ^^ₜ T).
Proof with auto.
  intro n1. induction n1.
  - intros. inversion H.
  - intros n2. induction n2.
    ...
    + intros. dependent destruction H2; rename x into Heq.
      ...
      * destruct A; simpl in *; try solve [ inversion Heq].
        -- destruct n.
            ++ unfold open_typ_wrt_typ in Heq. simpl in *.
                subst. dependent destruction H4. unify_binds.
            ++ unfold open_typ_wrt_typ in Heq. simpl in *.
                inversion Heq.
        -- inversion Heq. subst. rewrite <- Heq in H5.
           dependent destruction H5. unify_binds.
      * destruct A; simpl in *; try solve [ inversion Heq].
        -- destruct n.
           ++ unfold open_typ_wrt_typ in Heq. simpl in *.
           ...
           ++ unfold open_typ_wrt_typ in Heq. simpl in *.
              inversion Heq.
        -- simpl. inversion Heq. subst. inst_cofinites_for typ_bl__all; intros.
           fold open_typ_wrt_typ_rec.
           eapply typ_bot_like_subst_inv with (X:=X0) (T:=T0); eauto.
           ++ apply d_sub_d_wf_typ1 in H7. apply d_wf_typ_lc_typ in H7.
              eapply lc_typ_subst_inv with (X:=X0) (T:=T0); eauto.
              rewrite subst_tvar_in_typ_open_typ_wrt_typ_tvar2; eauto. rewrite ftvar_in_typ_open_typ_wrt_typ_rec_upper...
           ++ rewrite subst_tvar_in_typ_open_typ_wrt_typ_tvar2; eauto. 
              simpl. inst_cofinites_with X0.
              destruct H5. 
              ** eapply IHn1 with (Ψ:=Ψ) (T:=T0); eauto.
                 erewrite d_open_mono_same_order; eauto. lia. 
                 erewrite <- subst_tvar_in_typ_open_typ_wrt_typ_tvar2 with (X:=X0); eauto.
                 --- rewrite_env (map (subst_tvar_in_dbind T0 X0) nil ++ Ψ).  eapply d_sneq_stvar_subst_tvar; eauto.
                     constructor... apply d_sub_d_wf_env in H7...
                 --- rewrite ftvar_in_typ_open_typ_wrt_typ_rec_upper...
              ** dependent destruction H5. pick fresh X1. inst_cofinites_with X1.
                 erewrite <- subst_tvar_in_typ_open_typ_wrt_typ_tvar2 with (X:=X1); eauto. 
                 eapply typ_bot_like_subst with (Ψ:=Ψ); eauto. apply d_mono_typ_d_wf_typ; eauto. 
                 rewrite ftvar_in_typ_open_typ_wrt_typ_rec_upper...
              ** rewrite ftvar_in_typ_open_typ_wrt_typ_rec_upper...
      * destruct A; simpl in *; try solve [inversion Heq].
        -- destruct n.
      ...
      * destruct A; simpl in *; try solve [inversion Heq].
        -- destruct n.
           ++ unfold open_typ_wrt_typ in Heq. simpl in Heq.
              subst. dependent destruction H4.
           ++ inversion Heq.
        -- inversion Heq. subst. apply typ_bl__intersection2. apply d_sub_d_wf_typ1 in H2; eauto. 
            eapply IHn2; eauto.
           ++ unfold open_typ_wrt_typ. lia.
           ++ unfold open_typ_wrt_typ. lia.
           ++ dependent destruction H5. auto.
...
(* *** *)
Theorem typ_bot_like_sub_all : forall n1 n2 Ψ A B,
  d_typ_order A < n1 ->
  d_typ_size A < n2 ->
  typ_bot_like A ->
  ⊢ᵈ Ψ ->
...
Proof.
  ...
  - dependent destruction H1; auto.
    + simpl in *. dependent destruction H4. constructor; eauto.
      apply IHn2; eauto. lia. lia.
    + simpl in *. dependent destruction H4. apply d_sub__intersection3; eauto.
      apply IHn2; eauto. lia. lia.
    + simpl in *. dependent destruction H3. constructor; auto.
      apply IHn2; eauto. lia. lia.
      apply IHn2; eauto. lia. lia.
    + dependent induction H4; auto. 
      * inst_cofinites_for d_sub__alll T:=typ_unit; eauto.
        eapply IHn1; eauto.
        -- simpl in *. erewrite d_open_mono_same_order; eauto. lia.
        -- pick fresh X. erewrite <- subst_tvar_in_typ_open_typ_wrt_typ_tvar2; eauto.
          inst_cofinites_with X. eapply typ_bot_like_subst; eauto. 
      ...
      * inst_cofinites_for d_sub__alll T:=typ_unit; eauto.
        eapply IHn1; eauto.
        -- simpl in *. erewrite d_open_mono_same_order; eauto. lia.
        -- pick fresh X0. erewrite <- subst_tvar_in_typ_open_typ_wrt_typ_tvar2; eauto.
           inst_cofinites_with X. eapply typ_bot_like_subst; eauto. 
        -- eapply d_wf_typ_all_open; eauto.
      * inst_cofinites_for d_sub__alll T:=typ_unit; eauto.
        eapply IHn1; eauto.
      ...
      * inst_cofinites_for d_sub__all; intros; inst_cofinites_with X.
        -- right. inst_cofinites_for typ_bl__all. intros. 
           erewrite <- subst_tvar_in_typ_open_typ_wrt_typ_tvar2 with (X:=X); eauto.
           eapply typ_bot_like_rename. eauto. 
        -- eapply IHn1; eauto.
           ++ simpl in *. rewrite d_open_svar_same_order. lia.
           ++ dependent destruction H3. pick fresh X0. inst_cofinites_with X0.
              erewrite <- subst_tvar_in_typ_open_typ_wrt_typ_tvar2 with (X:=X0); eauto.
              apply d_wf_typ_rename_stvar_cons.
...
Proof with subst; eauto using d_wf_typ_weaken_app.
  ...
  - simpl. inst_cofinites_for d_sub__all; intros X1 Hfr; inst_cofinites_with X1.
    + destruct H. 
      * left. intros. split; rewrite subst_tvar_in_typ_open_typ_wrt_typ_fresh2...
        rewrite_env (map (subst_tvar_in_dbind C X) (X1 ~ □ ++ Ψ2) ++ Ψ1).
        apply d_sneq_stvar_subst_stvar; eauto.  apply d_sub_d_wf_env in H0.
        dependent destruction H0. dependent destruction H0. simpl. constructor; auto.
        intuition.
        rewrite_env (map (subst_tvar_in_dbind C X) (X1 ~ □ ++ Ψ2) ++ Ψ1).
        apply d_sneq_stvar_subst_stvar; eauto. apply d_sub_d_wf_env in H0.
        dependent destruction H0. dependent destruction H0.
        simpl. constructor; eauto. intuition.
      * right. replace (typ_all ({C ᵗ/ₜ X} A)) with ({C ᵗ/ₜ X} (typ_all A))...
        eapply typ_bot_like_subst; eauto.
    + repeat rewrite subst_tvar_in_typ_open_typ_wrt_typ_fresh2; eauto...
      rewrite_env (map (subst_tvar_in_dbind C X) (X1 ~ ■ ++ Ψ2) ++ Ψ1).
      eapply H1; auto...
  - simpl. destruct (neq_all_intersection_union_subst_stv B C X) as [[? [? ?]] | ?]; eauto...
    + inst_cofinites_for d_sub__alll T:=({C ᵗ/ₜ X} T); auto...
      * intros. rewrite subst_tvar_in_typ_open_typ_wrt_typ_fresh2... 
        inst_cofinites_with X0.  destruct H2. 
        -- left. rewrite_env (map (subst_tvar_in_dbind C X) (X0 ~ □ ++ Ψ2) ++ Ψ1).
           apply d_sneq_stvar_subst_stvar; eauto. apply d_sub_d_wf_env in H4. simpl. constructor; auto.
        -- right. replace (typ_all ({C ᵗ/ₜ X} A)) with ({C ᵗ/ₜ X} (typ_all A))...
           eapply typ_bot_like_subst; eauto.
    ...
    + simpl. destruct_eq_atom. pick fresh X0. inst_cofinites_with X0.
      destruct H2.
      * eapply d_sub_open_mono_stvar_sub_bot in H4 as Hbl; eauto.
        -- eapply typ_bot_like_all_inv in Hbl; eauto.
           eapply typ_bot_like_sub_all; eauto.
           ++ replace (typ_all ({C ᵗ/ₜ X} A)) with ({C ᵗ/ₜ X} (typ_all A))...
              eapply typ_bot_like_subst; eauto.
           ++ apply d_wf_env_subst_stvar...
              eapply d_sub_d_wf_env; eauto.
           ++ replace (typ_all ({C ᵗ/ₜ X} A)) with ({C ᵗ/ₜ X} (typ_all A))...
              eapply d_wf_typ_subst_stvar; eauto. 
              apply d_sub_d_wf_env in H4...
              apply d_sub_d_wf_typ1 in H4. 
              eapply d_wf_typ_all_inv; eauto.
           ++ apply d_wf_typ_weaken_app; eauto.
        -- rewrite <- subst_tvar_in_typ_open_typ_wrt_typ_tvar2 with (X:=X0); eauto.
           rewrite_env (map (subst_tvar_in_dbind T X0) nil ++ (Ψ2 ++ X ~ ■ ++ Ψ1)).
           apply d_sneq_stvar_subst_tvar; eauto. constructor... apply d_sub_d_wf_env in H4... 
      * eapply typ_bot_like_sub_all; eauto.
        -- replace (typ_all ({C ᵗ/ₜ X} A)) with ({C ᵗ/ₜ X} (typ_all A))...
           eapply typ_bot_like_subst; eauto.
        -- apply d_wf_env_subst_stvar; eauto. 
           apply d_sub_d_wf_env in H4...
        -- replace (typ_all ({C ᵗ/ₜ X} A)) with ({C ᵗ/ₜ X} (typ_all A))...
           eapply d_wf_typ_subst_stvar; eauto. 
           apply d_sub_d_wf_env in H4...
  ...
  - simpl. apply d_sub__union2. eauto.
    apply d_wf_typ_subst_stvar; auto...
    destruct (d_sub_d_wf _ _ _ H) as [? [? ?]]. auto.
  - simpl. inst_cofinites_for d_sub__all_refl. intros. inst_cofinites_with X0.
    destruct (d_sub_d_wf _ _ _ H) as [? [? ?]]. auto.
    apply d_sub_refl...
    + rewrite_env (map (subst_tvar_in_dbind C X) (X0 ~ ■ ++ Ψ2) ++ Ψ1).
      apply d_wf_env_subst_stvar; eauto.
    + rewrite_env (map (subst_tvar_in_dbind C X) (X0 ~ ■ ++ Ψ2) ++ Ψ1).
...
Inductive d_sub_size : denv -> typ -> typ -> nat -> Prop :=    (* defn d_sub *)
 ...
 | d_subs__all : forall (L:vars) (Ψ:denv) (A1 B1:typ) (n:nat),
      ( forall X , X \notin  L  ->  (  (  ( d_sneq_stvar  ( X ~ dbind_tvar_empty  ++  Ψ )   ( open_typ_wrt_typ A1 (typ_var_f X) )  ) /\ ( d_sneq_stvar  ( X ~ dbind_tvar_empty  ++  Ψ )   ( open_typ_wrt_typ B1 (typ_var_f X) )  )  )  ) \/ ( typ_bot_like (typ_all A1) )  )  ->
 ...
 | d_subs__alll : forall (L:vars) (Ψ:denv) (A1 B1 T1:typ) (n:nat),
     ...
     neq_intersection B1 ->
     neq_union B1 ->
     ( forall X , X \notin  L  ->  ( d_sneq_stvar  ( X ~ dbind_tvar_empty  ++  Ψ )   ( open_typ_wrt_typ A1 (typ_var_f X) )  ) \/ ( typ_bot_like (typ_all A1) )  )  ->
     d_mono_typ Ψ T1 ->
...
Proof with (simpl in *; eauto using d_wf_env_subst_tvar).
  ...
  - pick fresh SZ and apply d_subs__all. inst_cofinites_with SZ.
    + destruct H.
      * left. split.
        rewrite_env (map (subst_tvar_in_dbind (typ_var_f Y) X) (SZ ~ □ ++ Ψ2) ++ (Y, ■) :: Ψ1).
        -- rewrite subst_tvar_in_typ_open_typ_wrt_typ_fresh2; eauto.
           apply d_sneq_stvar_rename_dtvar; eauto.
           apply d_sub_size_sound in H0. apply d_sub_d_wf_env in H0.
           dependent destruction H0. dependent destruction H0. simpl. constructor; auto.  
           intuition.
        -- rewrite_env (map (subst_tvar_in_dbind (typ_var_f Y) X) (SZ ~ □ ++ Ψ2) ++ (Y, ■) :: Ψ1).
           rewrite subst_tvar_in_typ_open_typ_wrt_typ_fresh2; eauto.
           apply d_sneq_stvar_rename_dtvar; eauto. apply d_sub_size_sound in H0. apply d_sub_d_wf_env in H0.
           dependent destruction H0. dependent destruction H0. simpl. constructor; auto. intuition.
      * right. replace (typ_all ({typ_var_f Y ᵗ/ₜ X} A1)) with ({typ_var_f Y ᵗ/ₜ X} (typ_all A1)) by auto.
        apply typ_bot_like_rename...
  ...
  - pick fresh SZ and apply d_subs__alll.
    ...
    1-3: auto...
    + inst_cofinites_with SZ. 
      destruct H2.  left. rewrite subst_tvar_in_typ_open_typ_wrt_typ_fresh2...
      rewrite_env (map (subst_tvar_in_dbind ` Y X) (SZ ~  □ ++ Ψ2) ++ (Y, ■) :: Ψ1).
      eapply d_sneq_stvar_rename_dtvar...
      right. replace (typ_all ({` Y ᵗ/ₜ X} A1)) with ({` Y ᵗ/ₜ X} (typ_all A1)) by auto.
      apply typ_bot_like_rename...
  ...
  - applys d_subs__union2; forwards: IHHS...
    forwards: d_wf_typ_rename_stvar Y H...
  - inst_cofinites_for d_subs__all_refl. intros. inst_cofinites_with X0.
    rewrite subst_tvar_in_typ_open_typ_wrt_typ_fresh2... 
...
Proof with eauto.
  ...
  - destruct IHd_sub1 as [n1]. destruct IHd_sub2 as [n2].
    eauto...
  - pick fresh X.
    destruct (H1 X) as [n]. solve_notin.
    exists (S n).
    eapply d_subs__all with (L:=L `union` ftvar_in_typ A `union` ftvar_in_typ B `union` (dom Ψ)); intros; eauto.
    + rewrite_env (nil ++ X0 ~ ■ ++ Ψ). rewrite_env (nil ++ X ~ ■ ++ Ψ) in H2.
      applys d_sub_size_rename H2; solve_notin.
  ...
  - dependent destruction Hsub1...
    ...
    + simpl in *. dependent destruction Hsub2...
      * econstructor...
        ...
        ** inst_cofinites_with Y. apply d_sub_size_sound in H0.
           apply d_sub_d_wf_typ1 in H0. apply d_wf_typ_stvar_tvar_cons. auto.
      * inst_cofinites_for d_sub__all; auto;
        intros; inst_cofinites_with X.
        -- destruct H; destruct H1.
           ++ left. intuition.
           ++ right. apply d_sub_size_sound in H0.
              apply d_sub_bot_like_bot_like in H0; eauto.
              ** inst_cofinites_for typ_bl__all. intros.
                 erewrite <- subst_tvar_in_typ_open_typ_wrt_typ_tvar2 with (X:=X); eauto. apply typ_bot_like_rename; auto.
              ** dependent destruction H1. pick fresh X0. inst_cofinites_with X0.
                 erewrite <- subst_tvar_in_typ_open_typ_wrt_typ_tvar2 with (X:=X0); eauto. apply typ_bot_like_rename; auto.
           ++ right. auto.
           ++ right. auto.
        -- simpl. eapply IHn_d_typ_order with (B:= B1 ᵗ^ₜ X); eauto...
           rewrite d_open_svar_same_order...
      * pick fresh Y and apply d_sub__alll. 5: applys H5. all: eauto.
        ** inst_cofinites_with Y. destruct H; intuition. 
        ** pick fresh X. inst_cofinites_with X.
           replace Ψ with (map (subst_tvar_in_dbind T1 X) nil ++ Ψ) by auto.
          rewrite_env  (nil ++ (X, ■) :: Ψ) in H0.
          eapply d_sub_size_subst_stvar with (C:=T1) in H0; simpl; eauto.
          destruct H0 as [n' Hsub].
      ...
      * eapply d_sub__union2.
        -- eapply IHn_d_sub_size with (B:=typ_all B1) (n1:=S n) (n2:=n0); eauto...
        -- auto.
      * inst_cofinites_for d_sub__all; intros; inst_cofinites_with X; auto.
    ...
    + simpl in *. assert (Ψ ᵗ⊢ᵈ B1) as Hwft1 by applys* d_sub_dwft_sized_1.
      ...
      * apply d_sub_size_sound in Hsub1.
        eapply d_sub_open_mono_bot_sub_bot in Hsub1 as Hbl; eauto.
        eapply typ_bot_like_all_inv in Hbl; eauto.
        eapply typ_bot_like_sub_all; auto.
        -- apply d_sub_d_wf_env in Hsub1; auto.
        -- apply d_sub_d_wf_typ1 in Hsub1. inst_cofinites_for d_wf_typ__all. intros. rewrite_env (nil ++ X ~ □ ++ Ψ).  
           eapply d_wf_typ_open_mono_inv; eauto.
        -- apply d_sub_size_sound in Hsub2. apply d_sub_d_wf_typ2 in Hsub2. auto.
      * dependent destruction Hsub2; try solve ord_inv; auto...
        -- econstructor...
           pick fresh Y and apply d_wf_typ__all.
           forwards HW: d_sub_dwft_sized_1 Hsub1.
      ...
      * dependent destruction Hsub2; try solve ord_inv; auto...
      ...
      (* forall a. A < X < C *)
      * apply d_sub_size_sound in Hsub1.
        pick fresh X0. inst_cofinites_with X0. destruct H2.
        -- eapply d_sub_open_mono_stvar_sub_bot in Hsub1 as Hbl; eauto.
           eapply typ_bot_like_all_inv in Hbl; eauto.
           eapply typ_bot_like_sub_all; auto.
           ++ apply d_sub_d_wf_env in Hsub1; auto.
           ++ apply d_sub_d_wf_typ1 in Hsub1. inst_cofinites_for d_wf_typ__all. intros. rewrite_env (nil ++ X1 ~ □ ++ Ψ).  
              eapply d_wf_typ_open_mono_inv; eauto.
           ++ apply d_sub_size_sound in Hsub2. apply d_sub_d_wf_typ2 in Hsub2. auto.
           ++ rewrite <- subst_tvar_in_typ_open_typ_wrt_typ_tvar2 with (X:=X0); eauto.
              rewrite_env (nil ++ (X0 ~ □) ++ Ψ) in H2.
              eapply d_sneq_stvar_subst_tvar in H2; eauto.
        -- eapply typ_bot_like_sub_all; auto.
           ++ apply d_sub_d_wf_env in Hsub1; auto.
           ++ apply d_sub_d_wf_typ1 in Hsub1. inst_cofinites_for d_wf_typ__all. intros. rewrite_env (nil ++ X1 ~ □ ++ Ψ).  
              eapply d_wf_typ_open_mono_inv; eauto.
