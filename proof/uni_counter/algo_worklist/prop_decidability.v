  ...
  - dependent destruction H.
    ...
    + simpl in *. dependent destruction Hm. dependent destruction H1.
      ...
      { intros A1 X L Heq HneqAll HneqInter HneqUnion Hnin. subst. dependent destruction H1.
        ...
        assert (Heq: iu_size (typ_all A1) = iu_size (A1 ᵗ^ₜ X)) by admit. (* safe *)
        eapply IHnw with (m := (3 * n0 + all_size (A1 ᵗ^ₜ X)) * (1 + iu_size A0) +
                                (3 * m0 + all_size A0) * (1 + iu_size (A1 ᵗ^ₜ X)) + S n); eauto; try lia.
        admit. (* safe: wf *)
        eapply measp_wl_conswork; eauto.
        assert (Heq': all_size (typ_all A1) = all_size (A1 ᵗ^ₜ X) + 1) by admit. (* safe *)
        rewrite Heq in Hlt. rewrite Heq' in Hlt. lia.
        assert (Heq': weight (A1 ᵗ^ₜ X) = weight A1) by admit. (* safe *)
      ...
      * dependent destruction H1.
        ...
        -- dependent destruction H3. dependent destruction H5.
           pick fresh X. inst_cofinites_with X.
           assert (Heq: all_size A = all_size (A ᵗ^ₜ X)) by admit. (* safe *)
           assert (Heq0: all_size A0 = all_size (A0 ᵗ^ₜ X)) by admit. (* safe *)
           assert (Heq': iu_size A = iu_size (A ᵗ^ₜ X)) by admit. (* safe *)
           assert (Heq0': iu_size A0 = iu_size (A0 ᵗ^ₜ X)) by admit. (* safe *)
           assert (Heq'': weight A = weight (A ᵗ^ₜ X)) by admit. (* safe *)
           assert (Heq0'': weight A0 = weight (A0 ᵗ^ₜ X)) by admit. (* safe *)
           ...
           { eapply IHnw with (m := (3 * n1 + all_size (A ᵗ^ₜ X)) * iu_size (A0 ᵗ^ₜ X) + (3 * n3 + all_size (A0 ᵗ^ₜ X)) * iu_size (A ᵗ^ₜ X) + n); eauto; simpl in *; try lia.
             admit. (* safe: wf *)
